...
intents.guild_messages  = True    # サーバー内のメッセージ
intents.dm_messages     = True    # DM のメッセージ
...
async def cmd_gpt(msg: discord.Message, user_text: str):
    ...
    # 履歴＋今回のメッセージから画像を全部抽出
    all_attachments = []
    for m in history + [msg]:
...
GPT_SYSTEM = (
    ...
    "produce a corrected LaTeX math expression that compiles. "
    "Respond with the corrected expression only."
)
...
async def render_latex_image(tex: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    # 環境判定
    is_env = any(e in tex for e in ["\\begin{align", "\\begin{gather", "\\begin{eqnarray"])
    if is_env:
        fontsize = 80
        figw, figh = 12, 4
    else:
        ...
        figw, figh = 6, 3
    # 仮描画
    fig0 = plt.figure(dpi=dpi, figsize=(figw, figh))
    fig0.patch.set_alpha(0)
    t0 = fig0.text(0, 0, tex, fontsize=fontsize, ha="left", va="bottom")
    try:
        fig0.canvas.draw()
        bb = t0.get_window_extent(renderer=fig0.canvas.get_renderer())
        w_in = bb.width / dpi * scale + 2 * pad
        h_in = bb.height / dpi * scale + 2 * pad
    except Exception as e:
        plt.close(fig0)
        return None, str(e)
    plt.close(fig0)
    ...
    fig.text(0.5, 0.5, tex, fontsize=fontsize, ha="center", va="center", transform=fig.transFigure)
    tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".png")
    png_path = tmp.name
    tmp.close()
    try:
    ...
    try:
        ...
        if mask.any():
            ...
            img = img.crop((
                max(x0 - margin, 0),
                max(y0 - margin, 0),
                min(x1 + margin, arr.shape[1]),
                min(y1 + margin, arr.shape[0])
            ))
            img.save(png_path)
    except Exception as e:
...
async def cmd_tex(msg, formula: str):
    ...
    if not formula:
        await msg.reply("👉 数式を入力してね！（例: y!tex E=mc^2）")
        return
...
""",
    ...
    if err:
        await msg.reply("⚙️ LaTeX のコンパイルに失敗…自動修正します！")
        fixed = await autofix_latex(formula, trim_text(err, 900))  # エラーは圧縮して渡す
        if fixed:
            tex_fixed = rf"\[{fixed}\]"
